...
const DefaultSelectOptionTwo = ({ options }: any) => {
  ...
  return (
    <ClickOutside onClick={() => setIsOpen(false)}>
      <div className="relative z-20 inline-flex cursor-pointer appearance-none rounded-md border border-stroke bg-white text-sm font-medium shadow-1 outline-none dark:border-dark-3 dark:bg-gray-dark dark:shadow-card-2">
        <div
          className={`py-2.5 pl-3.5 pr-9.5 text-sm font-medium text-dark dark:text-white ${isOpen ? "open" : ""}`}
          onClick={() => setIsOpen(!isOpen)}
        >
          {selectedOption}
          <span
            className={`absolute right-3 top-1/2 z-10 -translate-y-1/2 ${isOpen && "rotate-180"}`}
          >
            <svg
...
export default DefaultSelectOptionTwo;
